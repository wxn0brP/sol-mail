...
function checkVersionStatus() {
	fetch("/api/admin/version")
		.then(res => res.json())
		.then((ver: VersionInfo) => {
			if (!ver.success) return setVerInfoStatus("red");

			if (ver.isCurrent) return setVerInfoStatus("green");
			...
			verStatus.on("click", async () => {
				...
				const data = await fetch("/api/admin/auto-update").then(res =>
					res.json(),
				);
...
function checkClientLatestUpdate() {
	fetch("https://api.github.com/repos/wxn0brP/sol-mail-client/releases/latest")
		.then(res => res.json())
		.then(ver => {
...
qs("#toggle-software-notification").on("click", () =>
	softwarePopup.fadeToggle(),
);
