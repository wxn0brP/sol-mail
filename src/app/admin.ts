...
const checkAdmin: RouteHandler = async (req, _, next) => {
	...
	const dbUser = await db.master.users.findOne({
		_id: user._id,
	});
...
router.get("/users", async (_, res) => {
	const users = await db.master.users.find(
		{},
		{},
		{
			select: [
...
router.get("/files/:file", async (req, res) => {
	...
	if (!user || !name) {
		res.status(400);
		return res.json({
			message: "User and mail name are required",
		});
	}
	const filePath = join(
		"data",
		"files",
		sanitizeDirName(user),
...
router.get("/version", async (_, res) => {
	try {
		...
		try {
			...
			remoteSHA = execSync("git rev-parse origin/HEAD", {
				encoding: "utf-8",
			}).trim();
		} catch (error) {
			console.log(
				"Could not get remote SHA, continuing with local SHA only:",
				error,
			);
		}
...
router.delete("/subject", async (req, res) => {
	...
	await db.master.subjects.removeOne({
		_id: subject,
	});
	return res.json({
		err: false,
