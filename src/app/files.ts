...
console.log(
	`File upload limit: ${MAX_FILE_SIZE}B (${MAX_FILE_SIZE / 1024 / 1024}MB)`,
);
...
router.customParser("/:mailName", async (req, res) => {
	try {
		...
		if (!mailName) {
			...
			return res.json({
				err: true,
				msg: "Mail name is required",
			});
		}

		const userType = await db.master.users.findOne({
			_id: user._id,
		});
		const isAdmin = userType?.admin;
		if (isAdmin) user.name = "public";
		...
		if (
			fs.existsSync(
				path.join("data", "files", sanitizedUserName, sanitizedMailName),
			)
		) {
			res.writeHead(409, {
				Connection: "close",
			...
			return res.json({
				err: true,
				msg: "Mail directory already exists",
			});
		}
		...
		bb.on("file", (fieldname, file, { filename }) => {
			if (requestAborted) return file.resume();

			const mailDir = path.join(
				"data",
				"files",
				sanitizedUserName,
				sanitizedMailName,
			...
			file.on("limit", () => {
				console.warn(
					`File ${filename} exceeded the limit of ${MAX_FILE_SIZE / 1024 / 1024} MB.`,
				);
				requestAborted = true;
				rmSync(mailDir, {
					recursive: true,
		...
		bb.on("filesLimit", () => {
			...
			res.json({
				err: true,
				msg: `Too many files uploaded. Limit is ${MAX_FILES_COUNT}.`,
			});
		});
...
router.get("/files/:file", async (req, res) => {
	try {
		...
		if (!name) {
			res.status(400);
			return res.json({
				message: "Mail name are required",
			});
		}
