...
import { parseTimeToMs } from "./time.js";
export async function setToken(user, res) {
    ...
    if (res)
        res.setHeader("Set-Cookie", `token=${jwt}; Path=/; Secure=false; SameSite=Lax; Expires=${exp.toUTCString()}`);
    return {
        token: jwt,
        exp,
        expirationTime
